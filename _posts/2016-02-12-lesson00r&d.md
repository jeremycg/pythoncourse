...
[main website](http://www.jeremy.kiwi.nz/pythoncourse).

Moreover, in this lesson, it's assumed you either attended the Applied Stream intro to data
types lecture, and/or have read through [Lesson 00 - Applied Section](http://jeremy.kiwi.nz/pythoncourse/2016/02/09/lesson00applied.html).
...
(i) Spyder, which is the IDE we will use for the first few lessons. Tt allows coding and 
...
Finally, note that we are tracking both group and indiviaul progress, to make sure this course is 
...
In additional to the standard boolean opeartors (<,>,==,!=,) mentioned in the intro 
class, there are three others: `in` and `is` and `not`:
